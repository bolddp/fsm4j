...
/**
 * Generic FSM (Finite State Machine) implementation where all the states must
 * implement a common, generic FsmState interface that matches the state
 * machine.
 * 
 * The state machine behavior is based on a configuration, where each state has
 * a set of valid triggers that controls which state to enter next.
 * 
 * Each state should act as a self-containing module of functionality, only
 * interacting by accessing the state machine context and by firing triggers.
 * 
 * When entering a new state, a custom state resolver can be used to provide
 * dependency injection functionality in the state machine, enabling each state
 * to request additional dependencies by constructor injection.
 *
 * @param <TriggerType>
 *            The type that should be used as the trigger of the state machine,
 *            e.g. an enum or an integer.
 * @param <ContextType>
 *            The type of the context that should be used. The context is shared
 *            by all states of the state machine, so it's a natural place for
 *            sharing data and configuration that the states of the state
 *            machine need.
 */
public class StateMachine<TriggerType, ContextType> {
	...
	public void trigger(final TriggerType trigger) {
		...
		Class<? extends FsmState<TriggerType, ContextType>> sourceState = (currentStateConfiguration != null)
	...
	public void setListener(StateMachineListener<TriggerType, ContextType> listener) {
